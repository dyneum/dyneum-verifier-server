...
const { KYCCredential } = require("../utils/proofRequest");
...
const getKYCVerificationdQR = asyncFunction((req, res) => {
  // #swagger.tags = ['gender verificaion']
  const sessionId = req.query.sessionId ?? uuidv4();
  const tenant_id = req.query.tenant_id;
  const authorization = req.headers["authorization"];

  const uri = `${process.env.HOSTED_SERVER_URL}/kyc-verification/callback?sessionId=${sessionId}&tenant_id=${tenant_id}`;
...
const kycVerificationCallback = asyncFunction(async (req, res) => {
  // #swagger.tags = ['gender verificaion']
  ...
  try {
    ...
    const response = await postKYCVerification(
      sessionId,
      tokenStr,
      authorization,
      tenant_id
...
const postKYCVerification = async (
  sessionId,
  jwz,
  authorization,
  tenant_id
) => {
  const url = `${process.env.DYNEUM_SERVER}/api/v1/vendor-auth/kyc-verification-callback/`;
  const data = {
    jwz_token: jwz,
...
const getKYCVerificationToken = asyncFunction(async (req, res) => {
  const authorization = req.headers.authorization;
